...
def is_placeholder(x):
    # if not( type(x) is tf.Tensor and len(x.op.inputs) == 0 ):
        # print(x)
...
class BatchInput(PlacholderTfInput):
    def __init__(self, shape, dtype=tf.float32, name=None):
        ...
        """
        if shape == None:
            super().__init__(tf.placeholder(dtype, [None] , name=name))
        else:
...
class _Function(object):
    ...
    def __call__(self, *args, **kwargs):
        ...
        for inpt, value in zip(self.inputs, args):
            ...
            self._feed_input(feed_dict, inpt, np.array(value))
        # Update the kwargs
        # print("FUUUUUUUUUCK{}".format(feed_dict))
        ...
        # feed_dict[self.inputs[1]] = [feed_dict[self.inputs[1]]]
